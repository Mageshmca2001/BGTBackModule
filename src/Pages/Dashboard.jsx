...
const normalizeWeekRangeToDaily = (weekArray, referenceDate) => {
const dailyMap = {};
...
const testTypes = ['FunctionalTest', 'CalibrationTest', 'AccuracyTest', 'NICTest', 'FinalTest'];

for (const item of weekArray) {
const [startStr, endStr] = item.RecordDate.split(' to ');
...
const passKey = `${testType}_Pass`;
const failKey = `${testType}_Fail`;
const pass = Number(item[passKey] || 0);
const fail = Number(item[failKey] || 0);
const total = pass + fail;

const testKey = testType.replace('Test', '').toLowerCase();

// 📌 Distribute pass & fail evenly across days
const dailyPass = Math.round(pass / daysInRange);
const dailyFail = Math.round(fail / daysInRange);
const dailyTotal = dailyPass + dailyFail;

dailyMap[key][testKey] += dailyTotal;
dailyMap[key].total += dailyTotal;
dailyMap[key].completed += dailyPass;
}
}
}

// 🗓 Generate exactly Sunday–Saturday week
...
dailyCompleted.push(dailyMap[key]);
}

// 📊 Weekly Totals
const weeklyTotals = dailyCompleted.reduce((acc, day) => {
acc.total += day.total;
acc.completed += day.completed;
acc.functional += day.functional;
acc.calibration += day.calibration;
acc.accuracy += day.accuracy;
acc.nic += day.nic;
acc.finalInit += day.finalInit;
return acc;
}, {
total: 0,
completed: 0,
...
if (!countRes.ok || !presentWeekRes.ok || !hourlyRes.ok) throw new Error("Failed to fetch one or more datasets");
...
Functional: [], Calibration: [], Accuracy: [], NIC: [], Final: []
};

const testTypes = ['Functional', 'Calibration', 'Accuracy', 'NIC', 'FinalTest'];
const hours = ['06:00','07:00','08:00','09:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00','17:00','18:00','19:00','20:00','21:00','22:00','23:00','00:00','01:00','02:00','03:00','04:00','05:00','6.00'];
...
if (entry.Status === 'Total') map[start][testType].total = entry.MeterCount || 0;
else if (entry.Status === 'PASS') map[start][testType].pass = entry.MeterCount || 0;
});
});
return map;
};

const buildHourlyDetails = (map) => hours.map(time => {
const totals = {};
let completedSum = 0;
testTypes.forEach(testType => {
...
presentWeek: normalizeWeekRangeToDaily(rawPresentWeekData, today),
previousWeek: normalizeWeekRangeToDaily(rawPreviousWeekData, previousWeekStart),
hourlyDetails: {
...
functional={weekData?.functional || 0}
calibration={weekData?.calibration || 0}
accuracy={weekData?.accuracy || 0}
nic={weekData?.nic || 0}
finalInit={weekData?.finalInit || 0}
...
{/* Each Day Card */}
{weekData?.dailyCompleted?.map((item, i) => {
const dateObj = new Date(item.date || '');
const dayIndex = isNaN(dateObj.getDay()) ? i % 7 : dateObj.getDay();
