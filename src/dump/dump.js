...
const padWithNulls = (data) =>
  data.flatMap((value, i) => (i < data.length - 1 ? [value, null] : [value]));

const paddedLabels = labels.flatMap((label, i) =>
  i < labels.length - 1 ? [label, ''] : [label]
);

const completedPadded = padWithNulls(completedData);
const trackingPadded = padWithNulls(completedData); // optional
const thresholdPadded = padWithNulls(Array(completedData.length).fill(redLineValue));

const breakdownPadded = {};
breakdownFields.forEach((key) => {
  breakdownPadded[key] = padWithNulls(breakdownData[key] || []);
});
...
// Step 3: Create padded data
const completedPadded = padWithNulls(completedData);
const trackingPadded = padWithNulls(completedData); // same shape as completed
const thresholdPadded = padWithNulls(Array(completedData.length).fill(redLineValue));
...
// Step 4: Prepare padded breakdown
const breakdownPadded = {};
breakdownFields.forEach((key) => {
  breakdownPadded[key] = padWithNulls(breakdownData[key] || []);
});
